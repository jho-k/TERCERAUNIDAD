...
<div class="max-w-lg mx-auto p-6 bg-white shadow-lg rounded-lg border-2 border-black">
    <div class="bg-gradient-to-r from-green-500 to-emerald-600 p-5 rounded-t-lg">
        <h2 class="text-3xl font-semibold text-white text-center">Editar Medicamento</h2>
    </div>
        ...
        <form action="{{ route('medicamentos.update', [$receta->id_receta, $medicamento->id_medicamento_receta]) }}" method="POST">
            ...
            <div class="mb-4">
                ...
                <textarea name="instrucciones" id="instrucciones" rows="4"
                    class="w-full p-3 border border-black rounded-md bg-green-100 focus:ring-2 focus:ring-green-500 resize-y">{{ old('instrucciones', $medicamento->instrucciones) }}</textarea>
            </div>
